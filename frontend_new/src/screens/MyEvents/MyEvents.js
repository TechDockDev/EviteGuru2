...
const MyEvents = () => {
   ...
   return (
      <Box>
         <Stack direction={{ md: "row", lg: "row", sm: "column", xs: "column" }} justifyContent={"space-between"} alignContent={"center"}>
            <Box display={"flex"} justifyContent={"space-between"} alignItems={"center"} flexDirection={{ md: "row", lg: "row", sm: "column", xs: "column" }}>
            ...
            <Box display={"flex"} alignItems={"center"} justifyContent={"space-between"} px={1}>
               ...
               &nbsp;&nbsp;
               <Button variant="contained" size="small" startIcon={<AddIcon />} sx={{ color: "white" }}>
                  Add New
               </Button>
            </Box>
         </Stack>
                  ...
                  events?.map((event, index) => {
                     return (
                        <Grid
                           ...
                           <Typography sx={{ "&:hover": { fontWeight: "600" }, boxSizing: "border-box" }} width={"100%"} padding={"5px"} onClick={() => navigate("/dashboard/view-event", { state: { event } })}>
                              {event?.name}
                           </Typography>
                           <Box overflow={"hidden"} borderRadius={"8px"} position={"relative"}>
                              <Box
                                 ...
                                 <Button
                                    variant="text"
                                    onClick={() => navigate("/dashboard/view-event", { state: { event } })}
                                    sx={{
                                       color: "white",
                           ...
                           <Button disableElevation variant="contained" sx={{ color: "white", mt: 1 }}>
                              Customize
                           </Button>
                        </Grid>
                     );
