...
const PromotionalMail = () => {
   ...
   return (
      <Stack direction={"column"} spacing={2} m={3}>
         ...
         {type === "mail" ? (
            <>
               <TextField placeholder="Subject" name="subject" onChange={handleChange} required />
               <TextField multiline rows={10} name="body" placeholder="Body" onChange={handleChange} required />
            </>
         ) : (
            <TextField multiline rows={10} name="message" placeholder="Message" onChange={handleChange} />
         )}
         <Button disableElevation variant="contained" sx={{ color: "white" }} onClick={type === "mail" ? saveEmail : saveMessage}>
            Next
         </Button>
         <Button
            variant="outlined"
